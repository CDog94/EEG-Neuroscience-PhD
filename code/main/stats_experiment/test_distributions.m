...
% Report on all four moments
fprintf('\nMoment Statistics:\n');

% First moment - Mean
fprintf('Mean Statistics:\n');
fprintf('  Chi-Squared - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(chiSquaredResults.MeasuredMean), ...
    min(chiSquaredResults.MeasuredMean), ...
    max(chiSquaredResults.MeasuredMean));
fprintf('  Weibull - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(weibullResults.MeasuredMean), ...
    min(weibullResults.MeasuredMean), ...
    max(weibullResults.MeasuredMean));
fprintf('  Log-normal - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(lognormalResults.MeasuredMean), ...
    min(lognormalResults.MeasuredMean), ...
    max(lognormalResults.MeasuredMean));

% Second moment - Variance
fprintf('\nVariance Statistics:\n');
fprintf('  Chi-Squared - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(chiSquaredResults.MeasuredVariance), ...
    min(chiSquaredResults.MeasuredVariance), ...
    max(chiSquaredResults.MeasuredVariance));
fprintf('  Weibull - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(weibullResults.MeasuredVariance), ...
    min(weibullResults.MeasuredVariance), ...
    max(weibullResults.MeasuredVariance));
fprintf('  Log-normal - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(lognormalResults.MeasuredVariance), ...
    min(lognormalResults.MeasuredVariance), ...
    max(lognormalResults.MeasuredVariance));
...
fprintf('\nKurtosis Statistics:\n');
fprintf('  Chi-Squared - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(chiSquaredResults.MeasuredKurtosis), ...
    min(chiSquaredResults.MeasuredKurtosis), ...
    max(chiSquaredResults.MeasuredKurtosis));
fprintf('  Weibull - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(weibullResults.MeasuredKurtosis), ...
    min(weibullResults.MeasuredKurtosis), ...
    max(weibullResults.MeasuredKurtosis));
fprintf('  Log-normal - Mean: %.4f, Min: %.4f, Max: %.4f\n', ...
    mean(lognormalResults.MeasuredKurtosis), ...
    min(lognormalResults.MeasuredKurtosis), ...
    max(lognormalResults.MeasuredKurtosis));
end